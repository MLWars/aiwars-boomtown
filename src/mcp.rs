//! The MCP server for the Boomtown referee. A faithful copy of the chess warden's
//! `mcp.rs`, typed to `Match<Boomtown>`: a SINGLE bearer-gated `/mcp` endpoint
...
//! build (e.g. `build:housing`) instead of a UCI move.
...
use crate::boomtown::Boomtown;
use aiwars_mcp_warden::game::Match;

/// Shared, mutex-guarded match state shared across all agents' requests.
pub type SharedMatch = Arc<Mutex<Match<Boomtown>>>;

/// `make_move` tool arguments.
#[derive(Debug, Deserialize, JsonSchema)]
pub struct MakeMoveArgs {
    /// The lot to develop, one of your legal moves (e.g. "build:housing",
    /// "build:industry", "build:landmark", "build:park").
    pub mv: String,
...
impl RefereeHandler {
    #[tool(
        description = "Get the current city state: your prosperity, the four axis scores (population/income/prestige/stability), your weakest (most exposed) axis, the rival, the round, status, winner, and your legal builds.",
    ...
    #[tool(
        description = "List the lots you may develop now (build:housing / build:industry / build:landmark / build:park) and whether it is your turn, with the current ply.",
        annotations(read_only_hint = true, open_world_hint = false)
    )]
    ...
    #[tool(
        description = "Develop a lot. Pass the ply you saw as expected_ply. Housing→population, industry→income, landmark→prestige, park→stability; prosperity is a weighted blend. Booming one axis scales fast but is fragile — a seeded shock each round hits the strongest axis. Returns the new state on success; an error (no change) if illegal, out of turn, or stale.",
    ...
    #[tool(
        description = "Resign the build (forfeit). Your rival is awarded the win.",
        annotations(read_only_hint = false, destructive_hint = false, open_world_hint = false)
    )]
...
impl ServerHandler for RefereeHandler {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build()).with_instructions(
            "AIWars Boomtown referee. Call get_state / legal_moves to observe, make_move to develop \
             a lot (with the ply you saw), resign to forfeit. Build the most prosperous city across \
             the river by round seven: housing → population, industry → income, landmark → prestige, \
             park → stability. A seeded boom-or-bust shock hits the strongest axis each round — \
             diversify or gamble. Your seat is identified by your bearer token; you cannot act as \
             your rival.",
        )
    }
}
