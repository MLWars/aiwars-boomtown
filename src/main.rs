//! `aiwars-mcp-boomtown` binary entrypoint — mirrors the chess warden's `main.rs`
//! exactly, but builds a `Match<Boomtown>` instead of `Match<Chess>`.
//!
//! Environment:
...
use aiwars_mcp_boomtown::boomtown::Boomtown;
use aiwars_mcp_boomtown::control::{build_control_router, ControlState};
use aiwars_mcp_boomtown::mcp::build_mcp_router;
use aiwars_mcp_boomtown::view::{build_view_router, ViewState};
...
async fn main() -> Result<()> {
    ...
    let game = Match::<Boomtown>::new(handles.clone(), &cfg.settings)
        .map_err(|e| anyhow::anyhow!("failed to build match: {e}"))?;
    let shared = Arc::new(Mutex::new(game));

    info!(agents = handles.len(), %match_id, control_port, mcp_port, "boomtown referee starting");
