...
//! the chess warden's `view.rs`, typed to `Match<Boomtown>`. Serves `/state.json`
...
pub struct ViewState {
    pub game: Arc<Mutex<Match<Boomtown>>>,
}

/// `GET /state.json` — the live city JSON the SPA polls. A pure read.
...
mod tests {
    ...
    fn match_fixture() -> Arc<Mutex<Match<Boomtown>>> {
        let handles = vec!["sol".to_string(), "vera".to_string()];
        let m = Match::<Boomtown>::new(handles, &serde_json::json!({ "seed": 7 })).unwrap();
        Arc::new(Mutex::new(m))
    }
    ...
    #[tokio::test]
    async fn state_json_is_a_readable_city() {
        let resp = router()
        ...
        assert_eq!(v["game"], "boomtown");
        assert!(v["mayors"].is_array());
    }
